...
import 'package:roleplay_assistant/src/presentation/widgets/dice_roller.dart';
...
class _ToolsScreenState extends State<ToolsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_showDiceRoller ? 'Dice Roller' : 'Tools'),
        leading: _showDiceRoller
            ? IconButton(
                icon: const Icon(Icons.arrow_back),
                onPressed: () => setState(() => _showDiceRoller = false),
              )
            : null,
      ),
      body: _showDiceRoller
          ? const Center(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    // Provide a DiceRoller with some sensible defaults and
                    // allow the user to roll multiple dice in-place.
                    DiceRoller(count: 3, sides: 6, dieSize: 80),
                  ],
                ),
              ),
            )
